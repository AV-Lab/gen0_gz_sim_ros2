    ...
    def pedestrians_callback(self, msg):
        ...
        for i, pose in enumerate(msg.poses):
            ...
            for car_circle in self.car_circles:
                if self.circles_intersect((px, py), self.pedestrian_circle_radius, car_circle.center, self.car_circle_radius):
                    collision_detected[i] = True
                    break

            if not collision_detected[i]:
                for path_circle in self.path_circles:
                    if self.circles_intersect((px, py), self.pedestrian_circle_radius, path_circle.center, self.point_circle_radius):
                        collision_detected[i] = True
                        break
